...
def test_ingest_basic(monkeypatch, env):
    ...
    monkeypatch.setattr(prices.net, "request", lambda url, **kw: json.dumps(response).encode())
...
def test_ingest_no_data(monkeypatch, env):
    ...
    monkeypatch.setattr(
        prices.net, "request", lambda url, **kw: json.dumps({"chart": {"result": []}}).encode()
    )
...
def test_ingest_error(monkeypatch, env):
    ...
    monkeypatch.setattr(
        prices.net, "request", lambda url, **kw: (_ for _ in ()).throw(net.NetworkError("fail"))
    )
    ptot, atot = prices.ingest("AAPL")
    assert ptot == 0
