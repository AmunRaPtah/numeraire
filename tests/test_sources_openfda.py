...
def test_ingest(monkeypatch, env):
        ...
        "results": [
            {
                ...
                "products": [{"brand_name": "VENCLEXTA", "active_ingredients": [{"name": "venetoclax"}]}],
                "submissions": [
                    {
    ...
    monkeypatch.setattr(openfda.net, "request", lambda url, **kw: json.dumps(payload).encode())
...
def test_ingest_no_results(monkeypatch, env):
    ...
    monkeypatch.setattr(openfda.net, "request", lambda url, **kw: json.dumps({"results": []}).encode())
...
def test_ingest_404(monkeypatch, env):
    ...
    monkeypatch.setattr(
        openfda.net,
        "request",
        lambda url, **kw: (_ for _ in ()).throw(net.PermanentError("HTTP 404", status=404)),
    )
...
def test_ingest_error(monkeypatch, env):
    ...
    monkeypatch.setattr(
        openfda.net, "request", lambda url, **kw: (_ for _ in ()).throw(net.NetworkError("fail"))
    )
    result = openfda.ingest("ABBVIE", limit=10)
    assert result == ("", 0, 0)
